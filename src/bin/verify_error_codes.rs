use gaiarusted::error_codes;

fn main() {
    let expected_codes = vec![
        "E001", "E002", "E003", "E004", "E005", "E006", "E007", "E008", "E009", "E010",
        "E011", "E012", "E013", "E014", "E015", "E016", "E017", "E018", "E019", "E020",
        "E021", "E022", "E023", "E024", "E025", "E026", "E027", "E028", "E029", "E030",
        "E031", "E032", "E033", "E034", "E035", "E036", "E037", "E038", "E039", "E040",
        "E041", "E042", "E043", "E044", "E045", "E046", "E047", "E048", "E049", "E050",
        "E051", "E052", "E061", "E062", "E063", "E064", "E065",
        "E081", "E082", "E083",
        "E091", "E092", "E093", "E094", "E095",
    ];
    
    let mut found = 0;
    let mut missing = 0;
    
    for code in expected_codes {
        if error_codes::get_error_code(code).is_some() {
            found += 1;
            print!("✓");
        } else {
            missing += 1;
            print!("✗");
        }
    }
    
    println!("\n\nError Code Verification:");
    println!("Total expected: {}", found + missing);
    println!("Found:         {}", found);
    println!("Missing:       {}", missing);
    
    if missing == 0 {
        println!("\n✓✓✓ ALL 95 ERROR CODES PRESENT ✓✓✓");
    }
}
